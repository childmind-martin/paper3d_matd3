...
class Viewer(object):
    def __init__(self, width=500, height=500, display=None):
        ...
        self.max_matrix_stack = 100  # 限制矩阵栈的最大深度
        self.current_matrix_stack = 0  # 当前矩阵栈深度计数器
        ...
        @self.window.event
        def on_draw():
            self.current_matrix_stack = 0
        
        # 设置基本相机位置
        self.set_bounds(-2.2, 2.2, -2.2, 2.2)
        self.window.clear()
...
class Transform(Attr):
    ...
    def enable(self):
        # 获取Viewer实例的引用（可能是父对象）
        viewer = None
        for frame in inspect.stack():
            if 'self' in frame[0].f_locals:
                _self = frame[0].f_locals['self']
                if isinstance(_self, Viewer):
                    viewer = _self
                    break
        
        # 检查矩阵栈深度
        if viewer and hasattr(viewer, 'current_matrix_stack'):
            if viewer.current_matrix_stack >= viewer.max_matrix_stack:
                return  # 如果超过最大深度，则跳过此次变换
            viewer.current_matrix_stack += 1
    ...
    def disable(self):
        ...
        # 只有当栈深度在正常范围内才执行pop操作
        if viewer and hasattr(viewer, 'current_matrix_stack'):
            if viewer.current_matrix_stack > 0:
                glPopMatrix()
                viewer.current_matrix_stack -= 1
