...
# Reuse the exact shared eval spec from the latest R2S terminal-fix run.
...
if [ ! -f "$BASE_SPEC" ]; then
  echo "[r2s-clean-label] BASE_SPEC not found: $BASE_SPEC" >&2
  exit 1
fi
if [ ! -d "$R2S_SUMMARY_DIR" ]; then
  echo "[r2s-clean-label] R2S_SUMMARY_DIR not found: $R2S_SUMMARY_DIR" >&2
  exit 1
fi

export RUN_TAG="${RUN_TAG:-level3_r2s_clean_label_vs_behavior_seed101_ep1200_fixed_best_by_team_sr_official10x30_$(date +%Y%m%d_%H%M%S)}"
...
export MISSING_TRAIN_LABELS="${MISSING_TRAIN_LABELS:-matd3_cross_agent_ref_reward_to_success_selector_clean_label}"
...
export INTEGRATED_LABEL_ORDER="${INTEGRATED_LABEL_ORDER:-matd3_cross_agent_ref_reward_to_success_selector matd3_cross_agent_ref_reward_to_success_selector_clean_label}"
...
echo "[r2s-clean-label] run tag: $RUN_TAG"
echo "[r2s-clean-label] base spec: $BASE_SPEC"
echo "[r2s-clean-label] reused R2S summary: $R2S_SUMMARY_DIR"
echo "[r2s-clean-label] train/eval label: $MISSING_TRAIN_LABELS"
